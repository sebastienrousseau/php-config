...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/php-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [PER-CS 2.0 Formatting & Level 8 PHPStan Analysis](#per-cs-2-0-formatting-level-8-phpstan-analysis) — before and after rule comparison
...
Install `@sebastienrousseau/php-config` as a development dependency:
...
# pnpm
pnpm add -D @sebastienrousseau/php-config

# yarn
yarn add -D @sebastienrousseau/php-config

# bun
bun add -d @sebastienrousseau/php-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/php-config.git
cd php-config
make            # check + test
```
...
- **Target environment**: Optimized for PHP 8.3+ / PHPStan.
...
```bash
cp node_modules/@sebastienrousseau/php-config/.php-cs-fixer.php ./.php-cs-fixer.php
cp node_modules/@sebastienrousseau/php-config/phpstan.neon ./phpstan.neon
```

### Or point the tool at the installed file

```bash
php-cs-fixer fix --config node_modules/@sebastienrousseau/php-config/.php-cs-fixer.php
phpstan analyse --configuration node_modules/@sebastienrousseau/php-config/phpstan.neon
```
...
`@sebastienrousseau/php-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/php-config` solves this with three deliberate engineering choices:
...
## PER-CS 2.0 Formatting & Level 8 PHPStan Analysis

Enforces PHP-FIG PER Coding Style 2.0 and Level 8 static type inference with strict types and zero mixed types.
...
### After (@sebastienrousseau/php-config Enforced)
...
When working on legacy PHP 5.x/7.x projects without strict type declarations or modern PHPStan support.
