...
%global npm_bare      php-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-php-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable PHP configuration providing standardized PHP-CS-Fixer, PHPStan, and Psalm rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/php-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/php-config is a shareable configuration preset with no runtime
...
install -pm 0644 .php-cs-fixer.php %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 phpstan.neon %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
